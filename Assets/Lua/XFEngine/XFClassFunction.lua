...
function clone(object)
    ...
    local function _copy(object)
        ...
        -- logWarnWithCallStack("22222222222222222222222222222" .. tostring(getmetatable(object)))
        if type(getmetatable(object)) == "table" or getmetatable(object) == nil then
            -- body
            -- logWarnWithCallStack("5555555555555555555555" .. tostring(getmetatable(object)))
            -- only can be nil or table
...
function check_should_use_get(instance, key)
    local result = getmetatable(instance)
    local index = 1
    while result ~= nil do
        if result.__member[key] ~= nil and result.__member[key].get ~= "" then
            return true, index, result.__member[key].get
        end
...
---@return bool, number, string
function check_should_use_set(instance, key)
    local result = getmetatable(instance)
    local index = 1
    while result ~= nil do
        if result.__member[key] ~= nil and result.__member[key].set ~= "" then
            return true, index, result.__member[key].set
        end
        result = getmetatable(result)
        index = index + 1
    end
...
function register_members(class, key, getFuncName, setFuncName)
    class.__member[key] = {}
    class.__member[key].get = getFuncName
